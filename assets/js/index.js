...
let countNumberInputPassengerSell = document.getElementById(
  "input-passenger-sell"
);
...
document.addEventListener("DOMContentLoaded", () => {
  // Lấy tất cả cookie từ document.cookie
  ...
  cookies.forEach(cookie => {
      ...
      if (key === "user" && value === "admin1") {
          isAdmin = true;
      }
  });

  // Truy cập thẻ div.admin
  const adminDiv = document.querySelector(".admin");

  // Xử lý hiển thị dựa trên quyền
  if (isAdmin) {
      if (adminDiv) {
          adminDiv.style.display = "block"; // Hiển thị nếu là admin
      }
  } else {
      if (adminDiv) {
          adminDiv.style.display = "none"; // Đảm bảo ẩn nếu không phải admin
      }
  }
});
