document
  .getElementById("btn-submit").addEventListener("click", function (event) {
    ...
    const confirmPassword = document
      ...
      .value.trim(); // loại bỏ 2 đầu input type password thứ 2
    ...
    if (!username) {
      // nếu username để trống hiển thị
    ...
    if (isValid) { // kiểm tra nếu rằng isValid = true thì đăng kí thành công còn không thì hiển thị lỗi
...
document
  .querySelector("input[name='agree-term']")
  .addEventListener("change", checkCheckboxes); // thay đổi nếu có sự kiện

document
  .querySelector("input[name='newsletter']")
  .addEventListener("change", checkCheckboxes);
